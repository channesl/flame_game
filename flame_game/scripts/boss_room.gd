...
func _on_root_timer_timeout() -> void:
	match current_stage:
		0:
			spawn_roots_stage_1()
		1:
			%Root_Timer.wait_time = 1.5
			spawn_roots_stage_2()
		2:
			%Root_Timer.wait_time = time_between_roots
			spawn_roots_stage_1()
		3:
			%Root_Timer.wait_time = 3
			spawn_roots_stage_2()
			

func spawn_roots_stage_1():
	if roots_active:
		var random = RandomNumberGenerator.new()
		random.randomize()
		var rand = random.randi_range(1, 1)
		...
		$Timers/Root_Sound_Timer.wait_time = 3.4
		$Timers/Root_Sound_Timer.start()

func spawn_roots_stage_2():
	if roots_active:
		var new_root_scene = root.instantiate()
		%Roots.call_deferred("add_child", new_root_scene)
		new_root_scene.global_position = player.global_position - global_position
	
		
		$Timers/Root_Sound_Timer.wait_time = 0.4
		$Timers/Root_Sound_Timer.start()
...
func get_root_positions(position_set_number) -> Array:
	...
	match position_set_number:
		1: 
			for i in 6:
				positions_array.append(Vector2((((16*3) - 8) - (i*16)), ((16*2) - 8)))
				positions_array.append(Vector2((((16*3) - 8) - (i*16)), -((16*2) - 8)))
				if i <= 2:
					positions_array.append(Vector2(((16*3) - 8), (((16*1) - 8) - (i*16))))
					positions_array.append(Vector2(-((16*3) - 8), (((16*1) - 8) - (i*16))))
			...
			for i in 10:
				...
				if i <= 6:
					...
					positions_array.append(Vector2(-((16*5) - 8), (((16*3) - 8) - (i*16))))
			for i in 14:
				positions_array.append(Vector2((((16*7) - 8) - (i*16)), ((16*6) - 8)))
				positions_array.append(Vector2((((16*7) - 8) - (i*16)), -((16*6) - 8)))
				if i <= 9:
					positions_array.append(Vector2(((16*7) - 8), (((16*5) - 8) - (i*16))))
					positions_array.append(Vector2(-((16*7) - 8), (((16*5) - 8) - (i*16))))
		2: 
			for x in 4:
				for y in 12:
